...
from inference_case_by_patient import inference
from utils import misc_utils
from dataset.AIAI_features import AIAI_Feature
...
def get_dataloaders(config):
    train_loader = data.DataLoader(
        AIAI_Feature(data_path=config.data_path, mode='train',
                      modal=config.modal, feature_fps=config.feature_fps,
                      num_segments=config.num_segments, len_feature=config.len_feature,
                      seed=config.seed, sampling='uniform', supervision='weak', fold=config.fold, stage = config.stage),
        batch_size=config.batch_size,
        shuffle=True, num_workers=config.num_workers, drop_last=True)

    test_loader = data.DataLoader(
        AIAI_Feature(data_path=config.data_path, mode='test',
                      modal=config.modal, feature_fps=config.feature_fps,
                      num_segments=config.num_segments, len_feature=config.len_feature,
                      seed=config.seed, sampling='uniform', supervision='weak', fold=config.fold, stage = config.stage),
        batch_size=1,
    ...
    valid_loader = data.DataLoader(
        AIAI_Feature(data_path=config.data_path, mode='valid',
                      modal=config.modal, feature_fps=config.feature_fps,
                      num_segments=config.num_segments, len_feature=config.len_feature,
                      seed=config.seed, sampling='uniform', supervision='weak', fold=config.fold, stage = config.stage),
        batch_size=1,
...
class Trainer():
    def __init__(self, config):
        ...
        # data
        self.train_loader, self.valid_loader, self.test_loader = get_dataloaders(self.config)

        # loss, optimizer
        self.optimizer = torch.optim.RAdam(self.net.parameters(), lr=self.config.lr,
                                          betas=(0.9, 0.999), weight_decay=0.001)
        self.criterion = CrossEntropyLoss()
        self.Lgce = GeneralizedCE(q=self.config.q_val)
        self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(self.optimizer,T_max = 100, eta_min = 0.000001)
        ...
        self.config.detection_inf_step = len(self.train_loader.dataset) // self.config.batch_size
        ...
        with torch.no_grad():
            for data_loader in [self.train_loader, self.valid_loader, self.test_loader]:
                ...
                if data_loader.dataset.mode == 'test':
                    _mean_ap, test_acc, _, ap, auc = inference(self.net, self.config, data_loader, cls_gt=cls_gt,
                                                    model_file=self.config.model_file)
                    print("cls_acc={:.5f} map={:.5f} auc={:.5f}".format(test_acc * 100, _mean_ap * 100, auc))
                else:
                    _mean_ap, test_acc, _, ap = inference(self.net, self.config, data_loader, cls_gt=cls_gt,
                                                        model_file=self.config.model_file)
    ...
    def evaluate(self, epoch=0):
        if (self.step % self.config.detection_inf_step == 0)&(epoch>10):
            self.total_loss_per_epoch /= self.config.detection_inf_step
            with torch.no_grad():
                self.net = self.net.eval()
                mean_ap, test_acc, final_res, ap = inference(self.net, self.config, self.valid_loader,
            ...
            if test_acc > self.best_acc:
                ...
                with open(f_path, 'w') as f:
                    ...
                    f.write(string_to_write + '\n')
                    f.flush()
                    string_to_write = 'Accuracy = %0.2f' % test_acc
                    f.write(string_to_write + '\n')
                    f.flush()
    ...
    def train(self):
        for epoch in range(self.config.num_epochs):
            ...
            for _data, _label, _, _, _ in self.train_loader:
                ...
                cas_rgb, att_rgb, clu_rgb, base_rgb = self.net(_data)
                ...
                vid_loss = self.criterion(cas_top_rgb, _label) #+0.5 * self.criterion(cas_top_flow, _label)
                cost = vid_loss
                ...
                att = att_rgb #+ 0.5 * att_flow  # two-stream fusion (see suppl.), (N,)
                ...
                clu = clu_rgb #+ 0.5 * clu_flow  # two-stream fusion (see suppl.), (N, K^C)
                ...
                clu_loss = -torch.mean(torch.sum(clu_gt * F.log_softmax(clu_rgb, dim=1), dim=1)) 
                ...
                cls = cls_rgb  # two-stream fusion (see suppl.), (K^C, 2)
                ...
                cls_loss = -torch.mean(torch.sum(cls_gt * F.log_softmax(cls_rgb, dim=1), dim=1)) 
